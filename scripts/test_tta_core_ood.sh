#!/bin/bash
#
# TTA-Core evaluation runner — OOD setting (100% site shift)
...
echo "[INFO] MergeSlide-TTA-Core (no TCP, no L_task) — OOD"
...
run_to_logs \
    "$LOG_DIR/test_tta_core/result_tta_core_ood.log" \
    "$LOG_DIR/test_tta_core/error_tta_core_ood.log" \
    "$PYTHON_BIN" -u tools/run_classil_with_pt_features.py \
        --entrypoint    test_tta_core.py \
        --config        "$CONFIG" \
        --merge_model_path "$MERGED_DIR" \
        --swag_dir      "$SWAG_DIR" \
        --result_csv    "$LOG_DIR/tta_core_results_ood.csv" \
        --tta_stats_csv "$LOG_DIR/tta_core_stats_ood.csv" \
        --efficiency_json "$LOG_DIR/efficiency_tta_core_ood.json" \
        "${TTA_ABLATION_ARGS[@]}"
