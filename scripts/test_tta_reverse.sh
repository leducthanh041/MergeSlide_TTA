...
run_to_logs \
    "$LOG_DIR/result_tta_re_classil_tcp.log" \
    "$LOG_DIR/error_tta_re_classil_tcp.log" \
...
run_to_logs \
    "$LOG_DIR/result_tta_re_classil_naive.log" \
    "$LOG_DIR/error_tta_re_classil_naive.log" \
    "$PYTHON_BIN" -u test_tta.py \
        --config           "$CONFIG" \
        --save_dir         "$FINETUNED_DIR" \
        --merge_model_path "$MERGED_DIR" \
...
# ── TASK-IL — Reverse ─────────────────────────────────────────────────────────
run_to_logs \
    "$LOG_DIR/result_tta_re_taskil.log" \
    "$LOG_DIR/error_tta_re_taskil.log" \
    "$PYTHON_BIN" -u test_tta.py \
        --config           "$CONFIG" \
        --save_dir         "$FINETUNED_DIR" \
        --merge_model_path "$MERGED_DIR" \
