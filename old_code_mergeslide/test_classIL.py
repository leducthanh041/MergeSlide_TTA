...
def eval(test_loader, model, dict_class, device, prefix, save_location, prompt_prototypes, **kwargs):
    preds_all = []
    probs_all = []
    targets_all = []
    K = 400
    model = model.to(device)
    list_of_target_logits = []
    with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.bfloat16):
        for features, coords, label in test_loader:
            ...
            list_of_target_logits.append(float(logits[:, preds]))
    ...
    return eval_metrics, preds_all, targets_all, list_of_target_logits
...
if __name__ == "__main__":
    ...
    for fold_id in range(9, 10):
        fold = "fold_" + str(fold_id)
        merge_model_path = "/home/bui/continual_learning/WSIModelMerging/fusion_bench/method/opcm_no_rank_reverse/merged_weight_opcm_random_sampling_" + fold + ".pth"
        ...
        mlp = nn.Linear(768, sum(num_classes)).to(device)
        ...
        all_task_list_of_target_logits = {}
        for task_id in range(num_tasks):
            ...
            start = time.time()
            results, preds_all, targets_all, list_of_target_logits = eval(test_loader, model, dict_convert_class[task_id], device, prefix="", save_location=None, prompt_prototypes=classifier)
            all_task_list_of_target_logits[task_id] = list_of_target_logits
            end = time.time()
            ...
            f.write("acc_fold_" + str(fold_id) + str([acc_per_task[task_id] for task_id in acc_per_task]) + "\n")

            overall_time_per_task = (end - start) / len(test_loader)
        ...
        torch.save(all_task_list_of_target_logits, "logits_per_task_fold10_mergeslide.pth")
