...
def main() -> None:
    ...
    baseline_prompts_np = tta_model.task_prompts_source.detach().float().cpu().numpy()
    ...
    for task_id in range(cfg.training.num_tasks):
        ...
        for slide_index, (features, coords, label) in enumerate(tqdm(test_loader, desc=f"Task {task_id}", leave=False)):
            ...
            baseline_prompt_now = tta_model.task_prompts.detach().float().cpu().numpy()
            baseline_score = baseline_vec[None, :] @ baseline_prompt_now.T
    ...
    source_prompts_np = baseline_prompts_np.astype(np.float32)
